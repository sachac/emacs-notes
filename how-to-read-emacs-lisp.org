...
When using =kbd=, whitespace doesn't usually matter, but it's easier to read if you use space to separate the keys to type. For more information about the syntax used by =kbd=, see =C-h f= (=describe-function=) for =kbd=, then follow the link to the documentation for =edmacro-mode=.
...
- Thanks
  - aidalgol, rryoumaa, shergill, taus, tali713, Fuco, @philandstuff, forcer, DaveP (detailed feedback, thanks!), Adrian Lewis, Andrzej P.
