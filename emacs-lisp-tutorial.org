...
** How to set up your own keybord shortcuts
   ...
   type =C-h k= followed by the binding. If it isn't, you will see a
   message in the echo area telling you that the binding =is
   undefined=. If it /is/ bound to a command, Emacs will bring up a
   =*Help*= window with documentation for the command.
